...
public class SpeedNotifier implements PaceNotifier.Listener, SpeakingTimer.Listener {
    ...
    public SpeedNotifier(Listener listener, PedometerSettings settings, PedometerUtils pedometerUtils) {
        mListener = listener;
    ...
    private void tellFasterSlower() {
        if (mShouldTellFasterslower && mPedometerUtils.isSpeakingEnabled()) {
            long now = System.currentTimeMillis();
            if (now - mSpokenAt > 3000 && !mPedometerUtils.isSpeakingNow()) {
                float little = 0.10f;
                float normal = 0.30f;
                float much = 0.50f;
                ...
                if (mSpeed < mDesiredSpeed * (1 - much)) {
                    mPedometerUtils.say("much faster!");
                }
                else
                if (mSpeed > mDesiredSpeed * (1 + much)) {
                    mPedometerUtils.say("much slower!");
                }
                else
                if (mSpeed < mDesiredSpeed * (1 - normal)) {
                    mPedometerUtils.say("faster!");
                }
                else
                if (mSpeed > mDesiredSpeed * (1 + normal)) {
                    mPedometerUtils.say("slower!");
                }
                else
                if (mSpeed < mDesiredSpeed * (1 - little)) {
                    mPedometerUtils.say("a little faster!");
                }
                else
                if (mSpeed > mDesiredSpeed * (1 + little)) {
                    mPedometerUtils.say("a little slower!");
                }
                else {
                    spoken = false;
                }
    ...
    public void speak() {
        if (mSettings.shouldTellSpeed()) {
            if (mSpeed >= .01f) {
                mPedometerUtils.say(("" + (mSpeed + 0.000001f)).substring(0, 4) + (mIsMetric ? " kilometers per hour" : " miles per hour"));
            }
        }
