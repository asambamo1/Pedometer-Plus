...
public class PaceNotifier implements StepListener, SpeakingTimer.Listener {
    ...
    public PaceNotifier(PedometerSettings settings, PedometerUtils pedometerUtils) {
        ...
        if (mLastStepTime > 0) {
            ...
            if (isMeaningfull && sum > 0) {
                ...
                if (mShouldTellFasterslower && !mPedometerUtils.isSpeakingEnabled()) {
                    if (thisStepTime - mSpokenAt > 3000 && !mPedometerUtils.isSpeakingNow()) {
                        float little = 0.10f;
                        float normal = 0.30f;
                        float much = 0.50f;
    ...
    public void speak() {
        if (mSettings.shouldTellPace()) {
            if (mPace > 0) {
                mPedometerUtils.say(mPace + " steps per minute");
            }
        }
    }
